...
export default function ItemDetails() {
  ...
  // DEMO ONLY: also show the panel when the demo flag is set
  const isDemoUnclaimedFlag = !!(item as any)?._demoUnclaimedFlag;
  ...
  const showUnclaimedAlert = item && isOwner && item.status === "Available" &&
    (isUnclaimedFor2Months(item.createdAt) || isDemoUnclaimedFlag);
              ...
              <div className="mb-5 p-4 rounded-xl border border-yellow-500/20" style={{ background: "rgba(255,203,5,0.06)" }}>
                {isDemoUnclaimedFlag && (
                  <div className="flex items-center gap-1.5 mb-2">
                    <span className="text-[10px] font-black tracking-widest px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400 border border-orange-500/30 uppercase">
                      🎮 Demo Mode
                    </span>
                    <span className="text-[10px] text-white/30 font-semibold">Simulated 2-month trigger</span>
                  </div>
                )}
