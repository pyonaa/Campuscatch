...
export default function EditItem() {
  ...
  const fetchItem = async () => {
    try {
      setIsLoading(true);
      if (id) {
        const item = await mockApi.getItem(id);
        if (item) {
          setFormData({
            itemName: item.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const preview of imagePreviews) {
        if (preview.startsWith("http")) imageUrls.push(preview);
      }
    ...
    <div className="min-h-screen">
      <header className="navbar-poke sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-xl text-white font-bold tracking-wide">Edit Item</h1>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" value={formData.itemName}
                ...
                className="input-poke w-full" placeholder="e.g., Blue Backpack" required />
            </div>
            ...
            <div>
              ...
              <select value={formData.category}
                ...
                className="input-poke w-full" required>
